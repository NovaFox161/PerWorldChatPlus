			...
			if (args.length < 3) {
					...
					if (sender.hasPermission("pwcp.chat.color")) {
						if (sender instanceof Player) {
							try {
								...
								//Color invalid.
								String msgOr = MessageManager.getMessages().get().getString("Notification.Color.Invalid");
								sender.sendMessage(MessageManager.getPrefix() + ChatColor.translateAlternateColorCodes('&', msgOr));
							}
						} else {
							//Player only, use more args.
							String msgOr = MessageManager.getMessages().get().getString("Notification.Args.TooFew");
							sender.sendMessage(MessageManager.getPrefix() + ChatColor.translateAlternateColorCodes('&', msgOr));
						}
					} else {
				...
				if (type.equalsIgnoreCase("alias")) {
					if (WorldDataManager.hasWorldData(type2)) {
						...
						String msgOr = MessageManager.getMessages().get().getString("Command.Set.Alias");
						String msg = msgOr.replaceAll("%world%", type2).replaceAll("%alias%", value);
						sender.sendMessage(MessageManager.getPrefix() + ChatColor.translateAlternateColorCodes('&', msg));
					} else {
						String msg = MessageManager.getMessages().get().getString("Notification.World.DoesNotExist");
						sender.sendMessage(MessageManager.getPrefix() + ChatColor.translateAlternateColorCodes('&', msg));
					}
					...
					if (sender.hasPermission("pwcp.chat.color.other")) {
						...
						if (target != null) {
							try {
								...
								String msgOr = MessageManager.getMessages().get().getString("Command.Set.Color.Other");
							...
							String msg = MessageManager.getMessages().get().getString("Notifications.Player.Offline");
							sender.sendMessage(MessageManager.getPrefix() + ChatColor.translateAlternateColorCodes('&', msg));
						}
					} else {
						sender.sendMessage(MessageManager.getPrefix() + MessageManager.getNoPermMessage());
